...
public class HomeFragment_1 extends BaseAppSlideFragment {
    ...
    public static HomeFragment_1 newInstance(int indexActivated) {
        HomeFragment_1 fragment = new HomeFragment_1();
    ...
    public void onInitView() {
        TabSetting.HOME_1.setup(viewpagerTab);

        final FragmentPagerItems pages = new FragmentPagerItems(mContext);
        for (int titleResId : TabSetting.HOME_1.tabs()) {
            FragmentPagerItem fragmentPagerItem = TabSetting.HOME_1.getFragmentPagerItem(mContext, titleResId);
            pages.add(fragmentPagerItem);
        }

        if (indexActivated >= TabSetting.HOME_1.tabs().length)
            indexActivated = 0;
